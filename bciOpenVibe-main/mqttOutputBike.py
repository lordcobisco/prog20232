...
class MyOVBox(OVBox):
    ...
    def bikeCicle(self):
        while self.start:
            if stim != '' and rpm != 0 and self.start:
                deviceStim['m'] = p1
    ...
    def initialize(self):
        ...
        global stim
        stim = 'cmd2dev'+str(int(self.setting['Stimulation Box Number']))
        deviceStim['t'] = int(self.setting['Pulse Width'])
        deviceStim['t'] = 1/int(self.setting['Stim Frequence (Hz)'])
        global p1
        global p2
        p1 = str(self.setting['Right Leg Stim'])
        p2 = str(self.setting['Left Leg Stimulation'])
        self.t2 = None
        print(self.mqttHostServer,self.mqttGate, )
        client1.connect(self.mqttHostServer,self.mqttGate)
        ...
        if len(self.input[1]) > 0:
            ...
            if OVStimulationSet == type(stimuli) and len(stimuli) == 1:
                ...
                if stimuli[0].identifier == OVTK_GDF_Feedback_Continuous:
                    self.start = True
                    self.t2 = threading.Thread(target = self.bikeCicle)
                    self.t2.start()
                else:
                    if self.t2:
                        del self.t2
