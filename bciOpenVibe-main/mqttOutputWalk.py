...
class MyOVBox(OVBox):
	...
	def walkCicle(self):
		while self.start:
			if self.stim1 != '' and self.stim2 != ''  and rpm != 0 and self.start:
				deviceStim1['m'] =  self.m1d
				self.client1.publish(self.stim1,json.dumps(deviceStim1))
				deviceStim2['m'] =  self.m1e
				self.client1.publish(self.stim2,json.dumps(deviceStim2))   
				print('Gate Phase: 0')
				time.sleep(1/(rpm*0.016666666666667)/4.0)

				deviceStim1['m'] =  self.m2d
				self.client1.publish(self.stim1,json.dumps(deviceStim1))
				deviceStim2['m'] =  self.m2e         
				self.client1.publish(self.stim2,json.dumps(deviceStim2))   
				print('Gate Phase: 1')
				time.sleep(1/(rpm*0.016666666666667)/4.0)

				deviceStim1['m'] =  self.m3d
				self.client1.publish(self.stim1,json.dumps(deviceStim1))
				deviceStim2['m'] =  self.m3e
				self.client1.publish(self.stim2,json.dumps(deviceStim2))             
				print('Gate Phase: 2')
				time.sleep(1/(rpm*0.016666666666667)/4.0)

				deviceStim1['m'] =  self.m4d
				self.client1.publish(self.stim1,json.dumps(deviceStim1))
				deviceStim2['m'] =  self.m4e
				self.client1.publish(self.stim2,json.dumps(deviceStim2))     
				print('Gate Phase: 3')
				time.sleep(1/(rpm*0.016666666666667)/4.0)
				if not self.start:
	...
	def initialize(self):
		...
		self.stim1 = 'cmd2dev'+str(int(self.setting['Stimulation Box Number 2']))
		deviceStim1['t'] = int(self.setting['Pulse Width'])
		deviceStim1['p'] = 1/int(self.setting['Stim Frequence (Hz)'])
		
		self.stim2 = 'cmd2dev'+str(int(self.setting['Stimulation Box Number']))
		deviceStim2['t'] = int(self.setting['Pulse Width'])
		deviceStim2['p'] = 1/int(self.setting['Stim Frequence (Hz)'])

		self.m1d = str((self.setting['Fase1'])).split(';')[0]
		self.m1e = str((self.setting['Fase1'])).split(';')[1]
		self.m2d = str((self.setting['Fase2'])).split(';')[0]
		self.m2e = str((self.setting['Fase2'])).split(';')[1]
		self.m3d = str((self.setting['Fase3'])).split(';')[0]
		self.m3e = str((self.setting['Fase3'])).split(';')[1]
		self.m4d = str((self.setting['Fase4'])).split(';')[0]
		self.m4e = str((self.setting['Fase4'])).split(';')[1]
		
		self.t2 = None
		print(self.mqttHostServer,self.mqttGate, )
		self.client1 = mqtt.Client()    # Identificacao do client1e
		self.client1.connect(self.mqttHostServer,self.mqttGate)
		self.t1 = threading.Thread(target = self.client1.loop_forever)
		self.t1.start()
		print("Inicializou")
	...
	def process(self):
		...
		if len(self.input[1]) > 0:
			stimuli = self.input[1].pop()
			if OVStimulationSet == type(stimuli) and len(stimuli) == 1:
				print(stimuli[0].identifier,OVTK_GDF_Feedback_Continuous)
				if stimuli[0].identifier == OVTK_GDF_Feedback_Continuous:
					self.start = True
					self.t2 = threading.Thread(target = self.walkCicle)
					self.t2.start()
				else:
					if self.t2:
						del self.t2
			...
			elif  OVStimulationSet == type(stimuli) and  len(stimuli) > 1:
				for item in stimuli:
					print(item.identifier)
					if item.identifier == OVTK_GDF_Feedback_Continuous:
						self.start = True
						self.t2 = threading.Thread(target = self.walkCicle)
						self.t2.start()
					else:
	...
	def uninitialize(self):
		...
		client1.publish(stim,json.dumps(deviceStim))
		return
